      ...
      <main className="flex-1 pt-16">
        ...
        {leadArticle ? (
          <Link href={`/${leadArticle.slug}`} className="block relative h-[85vh] min-h-[600px] overflow-hidden group">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-16 z-10">
              <div className="max-w-[800px]">
                <div className="inline-block bg-gradient-to-r from-emerald-500 to-teal-600 px-5 py-2 rounded-full text-xs font-bold tracking-widest uppercase mb-6">
                ...
                <span className="inline-flex items-center gap-3 bg-white/5 text-emerald-400 px-8 py-4 rounded-full font-bold text-sm shadow-2xl group-hover:bg-white/10 transition">
                  Read Article
          ...
          <section className="relative overflow-hidden py-20 px-6">
            <div className="absolute inset-0 bg-gradient-to-br from-slate-900/85 via-emerald-900/80 to-slate-900/85" />
            <div className="max-w-6xl mx-auto relative z-10 text-center">
              <h1 className="text-5xl md:text-7xl font-black mb-4">Top Private Equity Placement Agents</h1>
              <p className="text-xl md:text-2xl text-emerald-300 font-medium">Fundraising Advisory & Capital Markets Intelligence</p>
            </div>
          </section>
        )}

        {/* H1 Header Section */}
        <section className="bg-gradient-to-r from-slate-900 to-emerald-950 py-12 text-center">
          <div className="max-w-6xl mx-auto px-6">
            <h1 className="text-3xl md:text-4xl font-black mb-2">Top Private Equity Placement Agents</h1>
            <p className="text-lg text-emerald-300 font-medium">Fundraising Advisory & Capital Markets Intelligence</p>
          </div>
        </section>
        ...
        <section className="max-w-6xl mx-auto px-6 py-12">
          <p className="text-gray-400 leading-relaxed mb-10 max-w-4xl">
            Welcome to Placement Quest, the definitive resource for <strong className="text-white">private equity placement agents</strong>.
            We provide comprehensive coverage of the top PE placement agents, their fund mandates, LP relationships, and fundraising activity.
            Whether you're a GP selecting an advisor, an LP tracking the market, or a professional following industry trends,
            Placement Quest delivers the intelligence you need.
          </p>
                  ...
                  <Link key={article.id} href={`/${article.slug}`} className="block group">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      {article.article_angle && (
                        <span className="bg-emerald-500/15 text-emerald-400 px-2 py-0.5 rounded text-xs font-semibold uppercase">
                    ...
                    <h3 className="font-bold text-lg group-hover:text-emerald-400 transition line-clamp-2">{article.title}</h3>
                    {article.excerpt && (
        ...
        <section className="bg-white/[0.02] border-y border-white/10 py-12">
          <div className="max-w-6xl mx-auto px-6">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
              <div>
                <div className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 px-3 py-1.5 rounded-full text-xs text-emerald-400 mb-2">
              ...
              <Link href="/ecosystem" className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 px-5 py-2.5 rounded-lg font-semibold text-sm transition">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
                Explore Universe
        ...
        <section className="max-w-6xl mx-auto px-6 py-16">
          <h2 className="text-2xl font-bold mb-4 pb-3 border-b-4 border-emerald-600 inline-block">Latest News</h2>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {articles.map(article => (
              <Link key={article.id} href={`/${article.slug}`} className="block group hover:-translate-y-1 transition-transform duration-300">
                ...
                <h3 className="font-bold text-lg group-hover:text-emerald-400 transition leading-snug mb-2">{article.title}</h3>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {allAgents.map(agent => {
                ...
                return (
                  <Link
                    key={agent.id}
                    href={`/directory/${agent.slug}`}
                    className="bg-white/[0.03] border border-white/10 rounded-xl overflow-hidden hover:border-emerald-500/50 hover:shadow-lg hover:shadow-emerald-500/10 transition-all group"
                  >
                    <div className="h-24 bg-gradient-to-br from-emerald-200 to-teal-200 relative">
                      {img.url && img.type === 'image' ? (
                        <img src={img.url} alt={agent.name} className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center">
                          <span className="text-4xl font-black text-emerald-500 opacity-30">{agent.name?.charAt(0)}</span>
                        </div>
                      )}
                    ...
                    <div className="p-5 pt-4">
                      <h3 className="font-bold group-hover:text-emerald-400 transition">{agent.name}</h3>
                      ...
                      {agent.specializations && agent.specializations.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-3">
                          {agent.specializations.slice(0, 3).map((spec: string, i: number) => (
                            <span key={i} className="bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded-full text-[10px] font-medium">
                              {spec}
                            </span>
                          ))}
                        </div>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {sections.how_it_works.content.steps.map((step: { title: string; description: string }, i: number) => (
                    <div key={i} className="text-center">
                      <div className="w-12 h-12 bg-emerald-500/20 rounded-full flex items-center justify-center mx-auto mb-4 text-xl font-black text-emerald-400">
                        {i + 1}
                      </div>
          ...
          <section className="bg-white/[0.02] border-t border-white/10 py-24 px-6">
            <div className="max-w-5xl mx-auto">
                  ...
                  <div className="space-y-3">
                    {sections.fee_structures.content.standard_fees.map((fee: { fund_type: string; fee_range: string }, i: number) => (
                      <div key={i} className="flex justify-between items-center bg-white/5 rounded-lg px-5 py-4">
                        ...
                        <span className="font-bold text-emerald-400">{fee.fee_range}</span>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    {sections.fee_structures.content.retainer_models.map((model: { type: string; monthly_retainer?: string; success_fee?: string; retainer_period?: string; note?: string }, i: number) => (
                      <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-6">
                        <h4 className="text-lg font-bold mb-4 text-emerald-400">{model.type}</h4>
              ...
              {sections.fee_structures.content?.example && (
                <div className="bg-gradient-to-r from-emerald-500/20 to-teal-500/20 rounded-xl p-8 text-center">
                  <h3 className="text-xl font-bold mb-5">Example: {sections.fee_structures.content.example.fund_size} Fund</h3>
                  <div className="flex justify-center gap-8 flex-wrap">
                    <div>
                      <span className="block text-3xl font-black text-emerald-400">{sections.fee_structures.content.example.total}</span>
                  ...
                  {sections.selection.content.criteria.map((item: { name: string; description: string }, i: number) => (
                    <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-6">
                      <div className="flex items-center gap-3 mb-3">
                        <span className="w-8 h-8 bg-emerald-500/30 rounded-full flex items-center justify-center font-bold text-emerald-400 text-sm">
                          {i + 1}
                        </span>
              ...
              {sections.trends.content?.trends && (
                <div className="grid md:grid-cols-2 gap-6">
                  {sections.trends.content.trends.map((trend: { name: string; description: string }, i: number) => (
                    <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-6">
                      <div className="flex items-center gap-3 mb-3">
                        <span className="text-2xl font-black text-emerald-500/50">{String(i + 1).padStart(2, '0')}</span>
              ...
              {sections.why_use.content?.reasons && (
                <div className="grid md:grid-cols-2 gap-6">
                  {sections.why_use.content.reasons.map((reason: { title: string; description: string }, i: number) => (
                    <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-6">
                      <h3 className="text-lg font-bold mb-3 text-emerald-300">{reason.title}</h3>
        ...
        {/* CTA Section */}
        <section className="bg-gradient-to-r from-emerald-600 to-teal-600 py-24 text-center">
          <div className="max-w-3xl mx-auto px-6">
            <h2 className="text-4xl font-black mb-5">Track Fund Placements in Real-Time</h2>
            ...
            <Link
              href="/contact"
              className="inline-block bg-white text-emerald-600 px-10 py-4 rounded-lg font-bold hover:bg-white/90 transition"
            >
              Get Started
            </Link>
          </div>
