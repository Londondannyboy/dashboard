...
import { generateStaticSitemap, generateDynamicSitemap, combineSitemaps, GTM_STATIC_ROUTES } from '@quest/ui'
import { sql } from '../lib/db'

const BASE_URL = 'https://gtm.quest'
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    const companies = await sql`
      ...
      WHERE status = 'published' AND company_type = 'gtm_provider'
    ` as { slug: string; updated_at?: string }[]
