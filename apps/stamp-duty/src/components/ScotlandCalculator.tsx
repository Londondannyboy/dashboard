...
import { calculateScotlandLBTT, formatCurrency, getScotlandRateInfo, type BuyerType } from '../lib/stamp-duty'
...
const BUYER_TYPES: { value: BuyerType; label: string; description: string }[] = [
  ...
  { value: 'additional', label: 'Additional Property', description: 'Already own another property (+6% ADS)' },
]

const PRESET_PRICES = [200000, 300000, 400000, 500000, 750000, 1000000]

export function ScotlandCalculator() {
  ...
  const result = useMemo(() => calculateScotlandLBTT(price, buyerType), [price, buyerType])
  const rateInfo = useMemo(() => getScotlandRateInfo(buyerType), [buyerType])
            ...
            <div className="space-y-2">
              {rateInfo.bands.map((band, i) => (
                <div key={i} className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-blue-400">{band.rate}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-br from-blue-500/20 to-cyan-500/20 rounded-2xl p-6 border border-blue-500/30">
            <div className="text-center">
              <div className="text-sm text-slate-400 mb-1">LBTT to Pay</div>
            ...
            <div className="mt-6 pt-6 border-t border-slate-600/50">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  <div className="text-slate-400">Base LBTT</div>
                  <div className="text-white font-medium">
                    {formatCurrency(result.totalTax - result.adsAmount)}
                  </div>
                </div>
                {result.adsAmount > 0 && (
                  <div className="col-span-2">
                    <div className="text-slate-400">+6% ADS</div>
          ...
          {result.breakdown.length > 0 && (
            <div className="bg-slate-800/50 rounded-2xl p-6 border border-slate-700/50">
              <h3 className="text-lg font-semibold text-white mb-4">LBTT Breakdown</h3>
              <div className="space-y-3">
                ...
                {result.adsAmount > 0 && (
                  <div className="flex justify-between items-center text-sm pt-2 border-t border-slate-700">
                    <div>
                      <div className="text-amber-400">Additional Dwelling Supplement</div>
                      <div className="text-xs text-slate-500">{formatCurrency(price)} × 6%</div>
                    </div>
                    <div className="text-amber-400 font-medium">{formatCurrency(result.adsAmount)}</div>
                  </div>
                )}
                <div className="flex justify-between items-center pt-3 border-t border-slate-600">
                  <div className="text-white font-semibold">Total LBTT</div>
                  <div className="text-blue-400 font-bold text-lg">{formatCurrency(result.totalTax)}</div>
                </div>
              </div>
            </div>
          )}
