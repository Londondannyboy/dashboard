      ...
      <main className="flex-1 pt-16">
        ...
        <section className="py-12 px-6 border-b border-white/10">
          <div className="max-w-7xl mx-auto">
            <h1 className="text-4xl md:text-5xl font-black mb-4">Placement Agent Directory</h1>
            <p className="text-xl text-gray-400 max-w-3xl">
              Comprehensive directory of {companies.length}+ top private equity placement agents worldwide.
              Browse by specialization, region, or track record.
            </p>
          </div>
        </section>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {companies.map((company) => (
                  <Link
                    ...
                    className="bg-white/[0.03] border border-white/10 rounded-xl overflow-hidden hover:border-blue-500/50 transition group"
                  >
                    <div className="h-24 bg-gradient-to-br from-blue-600/20 to-indigo-600/20 flex items-center justify-center relative">
                      {company.logo_url ? (
                        <img
                        ...
                        <span className="text-4xl font-black text-blue-400/30">
                          {company.name.charAt(0)}
                        </span>
                      )}
                      {company.global_rank && (
                        <span className="absolute top-2 right-2 bg-blue-500 text-white text-xs px-2 py-0.5 rounded font-bold">
                          #{company.global_rank}
                        </span>
                      )}
                    </div>
                      ...
                      {company.specializations && company.specializations.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                          {company.specializations.slice(0, 3).map((spec) => (
                            <span
                              key={spec}
                              className="px-2 py-1 bg-blue-500/10 text-blue-300 rounded text-xs font-medium"
                            >
                              {spec}
                            </span>
                          ))}
