...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!article) {
    return {
      title: 'Article Not Found | Chief of Staff Quest',
    }
  }

  return {
    title: `${article.title} | Chief of Staff Quest`,
    description: article.excerpt || article.title,
    keywords: `chief of staff, executive assistant, ${article.article_angle || 'career advice'}`,
    openGraph: {
      title: article.title,
      description: article.excerpt || article.title,
      type: 'article',
...
function parseMarkdown(content: string): string {
  ...
  let html = content
    ...
    .replace(/\*(.*?)\*/g, '<em>$1</em>')
    // Links
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="text-blue-600 font-semibold underline hover:text-blue-800" target="_blank" rel="noopener noreferrer">$1</a>')
    // Line breaks
    ...
    // Blockquotes
    .replace(/^> (.*$)/gm, '<blockquote class="border-l-4 border-blue-600 pl-4 italic my-6 text-gray-600">$1</blockquote>')
