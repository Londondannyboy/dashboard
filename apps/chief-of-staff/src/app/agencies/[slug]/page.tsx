...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!agency) {
    return {
      title: 'Agency Not Found | Chief of Staff Quest',
    }
  }

  return {
    title: `${agency.name} | Chief of Staff Recruitment Agency UK`,
    description: agency.description || `${agency.name} - Leading chief of staff recruitment agency profile`,
    keywords: `${agency.name}, chief of staff recruitment, executive assistant recruitment, ${agency.headquarters || 'UK'}`,
  }
}
...
export default async function AgencyPage({ params }: PageProps) {
  ...
  const anchorTexts = [
    "View all chief of staff recruitment agencies",
    "Top executive assistant recruiters",
    "Best CoS recruitment agencies UK",
    "Complete list of EA recruiters",
    "Leading chief of staff agencies"
  ]
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <header className="bg-white rounded-lg shadow-sm p-8 mb-8">
            <div className="flex items-start justify-between">
              ...
              {agency.global_rank && (
                <div className="ml-8">
                  <div className="bg-gradient-to-br from-blue-600 to-blue-800 text-white rounded-lg p-6 text-center min-w-[120px]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <section className="bg-white rounded-lg shadow-sm p-8">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {agency.specializations.map((spec: string) => (
                      <span key={spec} className="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium bg-blue-50 text-blue-700 border border-blue-200">
                        {spec}
                      </span>
                    ))}
                  </div>
            ...
            <div className="space-y-8">
              ...
              {/* Contact CTA */}
              {websiteUrl && websiteUrl !== 'Unavailable from provided context' && (
                <section className="bg-blue-600 rounded-lg shadow-sm p-6 text-white">
                  <h3 className="text-lg font-bold mb-3">Contact {agency.name}</h3>
                  <p className="text-blue-100 text-sm mb-4">
                    Visit their website to learn more about their services and current opportunities.
                  </p>
                  <a
                    href={websiteUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block w-full bg-white text-blue-600 font-semibold py-3 px-4 rounded-lg text-center hover:bg-gray-100 transition"
                  >
                    Visit Website
                  </a>
                </section>
                  ...
                  <div className="space-y-3">
                    {Object.entries(regionalRanks).map(([region, rank]) => (
                      <div key={region} className="flex justify-between items-center">
                        ...
                        <span className="font-bold text-blue-600">#{String(rank)}</span>
                      </div>
                    ))}
                  </div>
                </section>
          ...
          {/* Back Link with SEO */}
          <div className="mt-12">
            <Link href="/agencies" className="inline-flex items-center text-blue-600 hover:text-blue-800 font-medium">
              &larr; {anchorText}
            </Link>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            <div>
              ...
              <p className="text-gray-400 text-sm">Your source for Chief of Staff and Executive Assistant recruitment insights.</p>
            </div>
            <div>
